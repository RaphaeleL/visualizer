...
static void heap_swap(SearchState *s, int a, int b) {
    int va = s->heap[a];
    int vb = s->heap[b];
    s->heap[a] = vb;
    s->heap[b] = va;
...
static void heap_up(SearchState *s, int idx) {
    while (idx > 0) {
        int parent = (idx - 1) / 2;
...
static void heap_down(SearchState *s, int idx) {
    while (true) {
        int left = 2 * idx + 1;
        int right = left + 1;
        int smallest = idx;
...
static void heap_push(SearchState *s, int cell) {
...
static int heap_pop(SearchState *s) {
...
static void heap_decrease_key(SearchState *s, int cell) {
