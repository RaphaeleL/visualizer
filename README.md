...
- `entries/` - Main visualization modules (maze, sort, plotter, hexdump) with separate `.h` and `.c` files
- `algorithms/` - Algorithm implementations (maze pathfinding, sorting algorithms)
- `libs/` - Third-party libraries (raylib, build.h)
- `out/` - Compiled object files (generated by build system)
